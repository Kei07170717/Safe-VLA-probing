...
Run from the analysis dir:
    python train_probes_2.py --acts-dir acts_task1 --gt-dir groundtruth --ratings safety_ratings.csv
    python train_probes_2.py --acts-dir acts_task1 --gt-dir groundtruth --ratings safety_ratings.csv --success-only --out-prefix probe_success
"""
import argparse
import glob
import pathlib
...
def main():
    ...
    ap.add_argument("--success-only", action="store_true",
                    help="train/test on successful episodes only")
    args = ap.parse_args()
    ...
    ratings["task_success"] = ratings["task_success"].astype(str).str.strip().str.lower()
    ...
    if args.success_only:
        ...
        print(f"--success-only: kept {len(ratings)}/{before} episodes "
              f"(rating range {ratings['safety_rating'].min():.0f}-{ratings['safety_rating'].max():.0f})")

    # --- Join ratings onto every step by episode_id (inner join drops filtered eps) ---
    gt = gt.merge(ratings[["episode_id", "safety_rating"]], on="episode_id", how="inner")
    gt = gt.sort_values("global_infer_id").reset_index(drop=True)
    print(f"After join: {len(gt)} rows / {gt['episode_id'].nunique()} episodes with safety ratings")

    # --- Load activations, aligned by global_infer_id ---
    act_files = sorted(glob.glob(f"{args.acts_dir}/act_*.npz"))
    if not act_files:
        raise SystemExit(f"No activation files found in {args.acts_dir}")
    ...
    np.savez(f"{args.out_prefix}_steering.npz",
             ...
             **{f"{s}_layer": np.array(steering_vectors[s]["layer"]) for s in streams})
    ...
    try:
        ...
        title_suffix = " (success-only)" if args.success_only else " (all episodes)"
        axes[0].set_title("Probe R² vs layer" + title_suffix)
        axes[0].set_xlabel("layer"); axes[0].set_ylabel("test R²")
        axes[1].set_title("Spearman ρ vs layer" + title_suffix)
        axes[1].set_xlabel("layer"); axes[1].set_ylabel("test ρ")
